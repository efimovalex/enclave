...
func TestNew(t *testing.T) {
	key, err := keys.New("test-key", time.Minute*10)
	assert.NoError(t, err)
	assert.NotNil(t, key)
	assert.Equal(t, "test-key", key.Name)
	assert.Equal(t, time.Minute*10, key.TTL)
...
func TestKeysPackUnpack(t *testing.T) {
	key, err := keys.New("pack-key", keys.DefaultKeyTTL)
	assert.NoError(t, err)
	assert.NotNil(t, key)
...
func TestEncryptKey(t *testing.T) {
	key, err := keys.New("encrypt-key", keys.DefaultKeyTTL)
	assert.NoError(t, err)
	assert.NotNil(t, key)
